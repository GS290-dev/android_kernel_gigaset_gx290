...
#define MIPITX_OUTREGBIT(addr, field, value)                                  \
	do {                                                                  \
		...
		val = (val & ~REG_FLD_MASK(field)) |                          \
		      (REG_FLD_VAL((field), (value)));                        \
	...
	if (level >= 0) {
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
			...
			unsigned long dsi_base_addr =
			    (unsigned long)DISPSYS_DSI0_BASE;
	...
	if (level >= 1) {
		for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module);
			...
			unsigned long mipi_base_addr =
			    (unsigned long)DISPSYS_MIPITX0_BASE;
#endif
			...
			for (k = 0; k < 32;
			     k += 16) { /* only dump first 32 bytes cmd */
				DDPDUMP(
				    "0x%04x: 0x%08x 0x%08x 0x%08x 0x%08x\n", k,
				      ...
				      unsigned int param)
{
	...
	if (status.SLEEPIN_DONE)
		_set_condition_and_wake_up(
		    &(_dsi_context[i].sleep_in_done_wq));
...
void DSI_enter_ULPS(enum DISP_MODULE_ENUM module)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		ret = wait_event_timeout(
		    waitq->wq, atomic_read(&(waitq->condition)), 2 * HZ);
		atomic_set(&(waitq->condition), 0);
		if (ret == 0) {
			pr_debug("dsi%d wait sleepin timeout\n", i);
...
void DSI_exit_ULPS(enum DISP_MODULE_ENUM module)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		atomic_set(&(waitq->condition), 0);
		if (ret == 0) {
			pr_debug("dsi%d wait sleepout timeout\n", i);
			DSI_DumpRegisters(module, 1);
			DSI_Reset(module, NULL);
		}
	}
			      ...
			      struct cmdqRecStruct *cmdq)
{
	...
	if (ret == 0) {
		pr_debug("dsi%d wait cmddone(not busy) timeout\n", i);
		DSI_DumpRegisters(module, 1);
		DSI_Reset(module, NULL);
	}
#else
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (dsi_params->mode == SYNC_EVENT_VDO_MODE ||
			...
			ASSERT(dsi_params->horizontal_backporch *
			       ...
			       9);
			horizontal_backporch_byte =
			    (dsi_params->horizontal_backporch * dsiTmpBufBpp -
			     10);
		}
			...
			if (dsi_params->ufoe_params.compress_ratio == 3) {
				...
				if (ufoe_internal_width % 3 == 0) {
					...
					temp_w = ((temp_w % 2) == 1)
						     ? (temp_w + 1)
						     : temp_w;
					ps_wc = temp_w * _dsi_ps_type_to_bpp(
							     dsi_params->PS);
				}
			} else { /* 1/2 */
				ps_wc = (w + w % 4) / 2 *
...
int MIPITX_IsEnabled(enum DISP_MODULE_ENUM module, struct cmdqRecStruct *cmdq)
{
	...
	if (module == DISP_MODULE_DSIDUAL) {
		ASSERT(MIPITX_INREGBIT(MIPI_BASE_ADDR(DISP_MODULE_DSI0) +
					   MIPITX_PLL_CON1,
				       FLD_RG_DSI_PLL_EN) ==
		       MIPITX_INREGBIT(MIPI_BASE_ADDR(DISP_MODULE_DSI1) +
					   MIPITX_PLL_CON1,
				       FLD_RG_DSI_PLL_EN));
		...
		ret = MIPITX_INREGBIT(MIPI_BASE_ADDR(module) + MIPITX_PLL_CON1,
				      FLD_RG_DSI_PLL_EN);
	}
				 ...
				 struct LCM_DSI_PARAMS *dsi_params)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (dsi_params->lane_swap_en) {
			...
			MIPITX_OUTREGBIT(
			    ...
			    pad_mapping[dsi_params->lane_swap
					    [i][MIPITX_PHY_LANE_0]] +
				1);
	...
	for (i = 0; i < 5; i++) {
		for (j = 0; j < 10; j++) {
			MIPITX_OUTREG32(
			    addr, ((mipitx_impedance_backup[i]) >> j) & 0x1);
			addr += 0x4;
		}
		/* 0xD8 = 0x300 - 0x228*/
		addr += 0xD8;
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		if (data_Rate != 0) {
			...
			if (data_Rate > 2500) {
				...
				pr_debug("dataRate is too low(%d)\n",
					data_Rate);
				ASSERT(0);
			}
			 ...
			 * PCW bit 8~15 = (pcw*256*256 -
			 * floor(pcw)*256*256)*256
			 */
			/* pcw = data_Rate*4*txdiv/(26*2);//Post DIV =4, so
			 * need
			 * data_Rate*4
			 */
			...
			tmp =
			    ((pcw & 0xFF) << 24) |
			    (((256 * (data_Rate * pcw_ratio % 26) / 26) & 0xFF)
			     << 16) |
			    (((256 *
			       (256 * (data_Rate * pcw_ratio % 26) % 26) /
			       26) &
			      0xFF)
			     << 8) |
			    ((256 *
...
void DSI_MIPI_clk_change(enum DISP_MODULE_ENUM module, int clk)
{
	...
	if (_is_power_on_status(module)) {
		if (clk != 0) {
			...
			tmp = ((pcw & 0xFF) << 24) |
			      (((256 * (clk * pcw_ratio % 26) / 26) & 0xFF)
			       << 16) |
			      (((256 * (256 * (clk * pcw_ratio % 26) % 26) /
				 26) &
				0xFF)
			       << 8) |
			      ((256 *
...
#define NS_TO_CYCLE(n, c) ((n) / (c))
	...
	timcon1.TA_SURE = (dsi_params->TA_SURE == 0)
			      ...
			      : dsi_params->TA_SURE;
	timcon1.TA_GO =
	    (dsi_params->TA_GO == 0) ? (0x4 * timcon0.LPX) : dsi_params->TA_GO;
	/* -------------------------------------------------------------- */
	...
	timcon2.CLK_TRAIL =
	    ((dsi_params->CLK_TRAIL == 0) ? NS_TO_CYCLE(0x60, cycle_time)
				 ...
				 struct cmdqRecStruct *cmdq)
{
	...
	if (cmdq)
		DSI_MASKREG32(cmdq, &DSI_REG[0]->DSI_INTSTA, 0x00000020,
			      0x00000000);
	...
	if (cmdq) {
		DSI_POLLREG32(cmdq, &DSI_REG[0]->DSI_INTSTA, 0x00000020,
			      0x00000020);
		DSI_MASKREG32(cmdq, &DSI_REG[0]->DSI_INTSTA, 0x00000020,
			      0x00000000);
	}
			       ...
			       UINT8 *buffer, UINT8 buffer_size)
{
	...
	if (buffer == NULL || buffer_size == 0) {
		pr_debug("DSI Read Fail: buffer=%p and buffer_size=%d\n",
	...
	if (DSI_REG[d]->DSI_MODE_CTRL.MODE) {
		...
		pr_debug("DSI Read Fail: DSI Mode is %d\n",
	...
	do {
		if (max_try_count == 0) {
			pr_debug("DSI Read Fail: try 5 times\n");
		...
		if (DSI_REG[d]->DSI_INTSTA.RD_RDY != 0 ||
			...
			pr_debug("Last DSI Read Why not clear irq???\n");
			pr_debug("DSI_CMDQ_SIZE  : %d\n",
				AS_UINT32(&DSI_REG[d]->DSI_CMDQ_SIZE));
			for (i = 0; i < DSI_REG[d]->DSI_CMDQ_SIZE.CMDQ_SIZE;
			     i++) {
				pr_debug("DSI_CMDQ_DATA%d : 0x%08x\n", i,
					AS_UINT32(&DSI_CMDQ_REG[d]->data[i]));
			}
			pr_debug("DSI_RX_DATA0	: 0x%08x\n",
				AS_UINT32(&DSI_REG[d]->DSI_RX_DATA0));
			pr_debug("DSI_RX_DATA1	: 0x%08x\n",
				AS_UINT32(&DSI_REG[d]->DSI_RX_DATA1));
			pr_debug("DSI_RX_DATA2	: 0x%08x\n",
				AS_UINT32(&DSI_REG[d]->DSI_RX_DATA2));
			pr_debug("DSI_RX_DATA3	: 0x%08x\n",
		...
		/* 3. Send cmd */
		t0.CONFG = 0x04; /* /BTA */
		/* / 0xB0 is used to distinguish DCS cmd or Gerneric cmd, is
		 * that Right???
		 */
		...
		ret = wait_event_timeout(
		    waitq->wq, atomic_read(&(waitq->condition)), WAIT_TIMEOUT);
		atomic_set(&(waitq->condition), 0);
		if (ret == 0) {
		...
		if (ret == 0) {
			...
			pr_debug("DSI Read Fail: dsi wait cmddone timeout\n");
			DSI_DumpRegisters(module, 2);
			DSI_Reset(module, NULL);
		}
	...
	if (DSI_REG[d]->DSI_MODE_CTRL.MODE) {
		...
		if (cmd < 0xB0) {
			if (count > 1) {
				...
				goto_addr = (unsigned long)(&DSI_VM_CMD_REG[d]
								 ->data[0]
								 .byte0);
		...
		if (cmd < 0xB0) {
			if (count > 1) {
				...
				goto_addr = (unsigned long)(&DSI_CMDQ_REG[d]
								 ->data[1]
								 .byte0);
				mask_para =
...
static void lcm_udelay(UINT32 us) { udelay(us); }
			  ...
			  struct LCM_DRIVER *lcm_drv)
{
	...
	if (module == DISP_MODULE_DSI0) {
		...
		if (lcm_param.lcm_cmd_if == LCM_INTERFACE_DSI0) {
			 ...
			 * DSI_set_cmdq_V22_Wrapper_DSIDual;
			 */
			    ...
			    void *params)
{
	...
	DSI_PS_Control(module, cmdq, &(_dsi_context[0].dsi_params), roi->width,
		       roi->height);
	DSI_Send_ROI(DISP_MODULE_DSI0, cmdq, roi->x, roi->y, roi->width,
		     roi->height);
}
...
static int _ddp_dsi_stop_dual(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	if (DSI_REG[0]->DSI_MODE_CTRL.MODE == CMD_MODE) {
		...
		ret = wait_event_timeout(_dsi_context[0].cmddone_wq.wq,
					 ...
					 WAIT_TIMEOUT);
		if (ret == 0) {
			pr_debug("dsi0 wait event for not busy timeout\n");
		...
		ret = wait_event_timeout(_dsi_context[1].cmddone_wq.wq,
					 ...
					 WAIT_TIMEOUT);
		if (ret == 0) {
			pr_debug("dsi1 wait event for not busy timeout\n");
...
int ddp_dsi_stop(enum DISP_MODULE_ENUM module, void *cmdq_handle)
{
	...
	if (DSI_REG[i]->DSI_MODE_CTRL.MODE == CMD_MODE) {
		...
		ret = wait_event_timeout(_dsi_context[i].cmddone_wq.wq,
					 ...
					 WAIT_TIMEOUT);
		if (ret == 0) {
			pr_debug("dsi%d wait event for not busy timeout\n", i);
			DSI_DumpRegisters(module, 1);
			DSI_Reset(module, NULL);
		}
	} else {
			...
			void *params)
{
	...
	if (mode == 0) { /* V2C */
		...
		/* 3. disable DSI EXT TE, only BTA te could work, reason
		 * unknown
		 */
		DSI_OUTREGBIT(cmdq_handle, struct DSI_TXRX_CTRL_REG,
		...
		DSI_MASKREG32(cmdq_handle, DISP_REG_CONFIG_MUTEX0_SOF, 0x7,
			      0x1); /* sof */
		DSI_MASKREG32(cmdq_handle, DISP_REG_CONFIG_MUTEX0_SOF, 0x1c0,
			      0x40); /* eof */
		...
		DSI_MASKREG32(cmdq_handle, DISP_REG_CONFIG_MUTEX0_EN, 0x1,
		...
		while (wait_count) {
			...
			if (DSI_REG[i]->DSI_STATE_DBG6.CMTRL_STATE == 0x1) {
				DISPDBG(
					"[C2V]after c2v switch, dsi fsm is idle\n");
				break;
			}
			lcm_mdelay(1);
			wait_count--;
		  ...
		  enum DDP_IOCTL_NAME ioctl_cmd, void *params)
{
	...
	case DDP_DSI_PORCH_CHANGE: {
		if (params == NULL)
			...
			DDPMSG("[%s] DDP_DSI_PORCH_CHANGE vfp=%d\n",
			       __func__, vfp);
			ddp_dsi_porch_setting(module, cmdq_handle, DSI_VFP,
					      vfp);
		}
		break;
	}
	case DDP_DSI_PORCH_ADDR: {
		if (params == NULL)
			...
			DDPMSG(
			    "[%s] DDP_DSI_PORCH_ADDR addr=0x%x\n",
			    __func__, addr);
...
int ddp_dsi_trigger(enum DISP_MODULE_ENUM module, void *cmdq)
{
	...
	if (module == DISP_MODULE_DSIDUAL &&
		...
		/* Reading one reg is only used for delay in order to pull down
		 * DSI_DUAL_EN.
		 */
		if (cmdq)
...
void dsi_analysis(enum DISP_MODULE_ENUM module)
{
	...
	for (i = DSI_MODULE_BEGIN(module); i <= DSI_MODULE_END(module); i++) {
		...
		DDPDUMP(
			"DSI%d LFR En:%d, LFR MODE:%d, LFR TYPE:%d, LFR SKIP NUMBER:%d\n",
...
{
	if (!_is_power_on_status(module)) {
		pr_debug("sleep dump is invalid\n");
		return 0;
	}
...
//yanrenjie added k607 6761 kernel4.9 code to sync the esd lcm problem 20190424 start
...
int ddp_dsi_build_cmdq(enum DISP_MODULE_ENUM module, void *cmdq_trigger_handle, enum CMDQ_STATE state)
{
	...
	if (cmdq_trigger_handle == NULL) {
		pr_debug("cmdq_trigger_handle is NULL\n");
		return -1;
	}
	...
	if (state == CMDQ_WAIT_LCM_TE) {
		...
		} else {pr_debug("wrong module: %s\n",ddp_get_module_name(module));
			return -1;
		}
	} else if (state == CMDQ_CHECK_IDLE_AFTER_STREAM_EOF) {
		/* need waiting te */
		if (module == DISP_MODULE_DSI0 ||module == DISP_MODULE_DSIDUAL) {	DSI_POLLREG32(cmdq_trigger_handle,	&DSI_REG[dsi_i]->DSI_INTSTA, 0x80000000,
		0);
		} else {pr_debug("wrong module: %s\n",	ddp_get_module_name(module));
			return -1;
		}
...
//&&(read_data1.byte3 == dsi_params->lcm_esd_check_table[i].para_list[3])
			...
			else if( dsi_params->lcm_esd_check_table[i].count == 10)
			{	
				...
				&& (read_data1.byte2 == dsi_params->lcm_esd_check_table[i].para_list[2]) && (read_data1.byte3 == dsi_params->lcm_esd_check_table[i].para_list[3]) 
				&& (read_data2.byte0 == dsi_params->lcm_esd_check_table[i].para_list[4]) && (read_data2.byte1 == dsi_params->lcm_esd_check_table[i].para_list[5])
				&& (read_data2.byte2 == dsi_params->lcm_esd_check_table[i].para_list[6]) && (read_data2.byte3 == dsi_params->lcm_esd_check_table[i].para_list[7]) 
				&& (read_data3.byte0 == dsi_params->lcm_esd_check_table[i].para_list[8]) && (read_data3.byte1 == dsi_params->lcm_esd_check_table[i].para_list[9]))
				{
				/* clear rx data */
				/* DSI_OUTREG32(NULL, &DSI_REG[dsi_i]->DSI_RX_DATA0,0); */
					printk ("%s esd check ok.\n", __func__);
	...
	case CLK_HS_PRPR:
		if (dsi_index == PM_DSI0) {
			DSI_OUTREGBIT(NULL, struct DSI_PHY_TIMCON3_REG,
				      DSI_REG[0]->DSI_PHY_TIMECON3,
				      CLK_HS_PRPR, timing.value);
		} else if (dsi_index == PM_DSI1) {
			DSI_OUTREGBIT(NULL, struct DSI_PHY_TIMCON3_REG,
				      DSI_REG[1]->DSI_PHY_TIMECON3,
				      CLK_HS_PRPR, timing.value);
		} else if (dsi_index == PM_DSI_DUAL) {
			DSI_OUTREGBIT(NULL, struct DSI_PHY_TIMCON3_REG,
				      DSI_REG[0]->DSI_PHY_TIMECON3,
...
int DSI_check_roi(void)
{
	...
	msleep(20);
	count =
	    DSI_dcs_read_lcm_reg_v2(DISP_MODULE_DSI0, NULL, 0x2a, read_buf, 4);
	msleep(20);
	...
	msleep(20);
	count =
	    DSI_dcs_read_lcm_reg_v2(DISP_MODULE_DSI0, NULL, 0x2b, read_buf, 4);
